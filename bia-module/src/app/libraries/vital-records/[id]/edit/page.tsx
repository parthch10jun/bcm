...
import { UpdateVitalRecordRequest, VitalRecordStatus, VitalRecord } from '@/types/vitalRecord';
import { vitalRecordService } from '@/services/vitalRecordService';
import { ArrowLeftIcon, CheckIcon } from '@heroicons/react/24/outline';

export default function EditVitalRecordPage() {
  ...
  const [vitalRecord, setVitalRecord] = useState<VitalRecord | null>(null);

  const [formData, setFormData] = useState<UpdateVitalRecordRequest>({
    recordName: '',
  ...
  const loadVitalRecord = async () => {
    try {
      setLoading(true);
      const data = await vitalRecordService.getById(id);
      setVitalRecord(data);
      setFormData({
        ...
        storageFormat: data.storageFormat || '',
        retentionPeriod: data.retentionPeriod || '',
        notes: data.notes || ''
      });
    } catch (err) {
      console.error('Error loading vital record:', err);
      setError('Failed to load vital record');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await vitalRecordService.update(id, formData);
      router.push('/libraries/vital-records');
    } catch (err: any) {
      console.error('Error updating vital record:', err);
      setError(err.message || 'Failed to update vital record');
    } finally {
      setSaving(false);
    }
  };

  const handleChange = (field: keyof UpdateVitalRecordRequest, value: any) => {
  ...
  if (loading) {
    return (
      <div className="h-full flex items-center justify-center bg-gray-50">
        <div className="text-xs text-gray-500">Loading vital record...</div>
      </div>
    );
  }
      ...
      <div className="h-full flex flex-col bg-gray-50">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="bg-red-50 border border-red-200 rounded-sm p-3">
            <p className="text-xs text-red-700">Vital record not found</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Edit Vital Record</h1>
          <p className="mt-0.5 text-xs text-gray-500">
            Update vital record information
          </p>
        </div>
      </div>
