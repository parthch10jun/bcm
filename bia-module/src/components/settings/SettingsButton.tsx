 ...
 * Uses Golf Saudi green theme for primary buttons
 */
...
/**
 * Button group container for Settings pages
 */
export function SettingsButtonGroup({ children, align = 'right', className = '' }: SettingsButtonGroupProps) {
  const alignStyles = {
    left: 'justify-start',
    right: 'justify-end',
  ...
  return (
    <div className={`flex items-center space-x-3 ${alignStyles[align]} ${className}`}>
      {children}
    </div>
  );
}
